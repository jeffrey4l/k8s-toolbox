...
type StudentLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Student, err error)
...
// List lists all Students in the indexer.
func (s *studentLister) List(selector labels.Selector) (ret []*v2.Student, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Student))
	})
	return ret, err
}
...
type StudentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.Student, error)
...
// List lists all Students in the indexer for a given namespace.
func (s studentNamespaceLister) List(selector labels.Selector) (ret []*v2.Student, err error) {
...
// Get retrieves the Student from the indexer for a given namespace and name.
func (s studentNamespaceLister) Get(name string) (*v2.Student, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("student"), name)
	}
	return obj.(*v2.Student), nil
}
